...
class Aauth {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		if($this->config_vars['ddos_protection'] && $this->config_vars['recaptcha_active'] && $this->get_login_attempts() > $this->config_vars['recaptcha_login_attempts']){
 		...
 		if($this->config_vars['login_with_name'] == TRUE AND filter_var($identifier,FILTER_VALIDATE_EMAIL) == FALSE){
	 ...
	 */
	public function create_user($email, $pass, $username = FALSE,$default_group = NULL,$nogroup = false) {
		...
		if ( $this->aauth_db->insert($this->config_vars['users'], $data )){
			...
			{				
				...
				if(is_null($default_group))
					$default_group = $this->config_vars['default_group'];

				$this->add_member($user_id, $default_group);
			}
	...
	public function teslogin($username,$password)
	{
		// echo $_SERVER['PHP_AUTH_DIGEST'];
		// echo $username.$password;
		return md5('tes:REST API:tes');
	}
