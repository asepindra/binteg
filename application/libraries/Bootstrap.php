...
class Bootstrap
{
    ...
    public static function form($htmlOptions = array(),$return)
    {
        return new TbForm($htmlOptions,$return);
    }
    ...
    public static function checkBoxList($name, $active, $list = array(), $htmlOptions = array()) 
    {
        ...
        $type = self::getValue($htmlOptions, 'type', 'checkBox');
            $options = self::getValue($htmlOptions, 'CssRadioButton', array());
    ...
    public static function checkBox($name, $label, $value = '', $options = array())
    {
        ...
        $classRadio = self::getValue($options, 'radioClass', 'radio-primary', false);
        $id = $name.$value;
        self::addAttributes('name', $name, $options);
        self::addAttributes('value', $value, $options);
        self::addAttributes('id', $id, $options);
        // $field = form_checkbox($options) .' '. $label;
        $field = form_checkbox($options);
        if($inline == true) {
            $html = self::openTag('div', array('class' => $classRadio.' radio-inline'));
            $html .= $field;
            $html .= self::openTag('label', array('class' => $type . '-inline','for'=>$name.$value));            
            $html .= $label;
            ...
            $html = self::openTag('div', array('class' => $type.' '. $classRadio));
            $html .= $field;
            $html .= self::openTag('label',array('for'=>$name.$value));
            $html .= $label;
            $html .= self::closeTag('label');
            $html .= self::closeTag('div');
        }
...
class TbForm extends Bootstrap
{
    ...
    public function __construct($config = array(),$return = false)
    {
        ...
        if($return)
            $this->formOpen = $this->formOpen($action, $config);
        else
